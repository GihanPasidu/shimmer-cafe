/**
 * Shimmer Cafe - Database Handler
 * Manages data persistence using JSON files in the data folder
 */
...
const db = (function() {
    ...
    const saveToJSON = function(filename, data) {
        ...
        try {
            ...
            // Create a download link for actual JSON file if needed
            if (window.saveToFileSystem) {
                const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' });
                const url = URL.createObjectURL(blob);
                const a = document.createElement('a');
                a.href = url;
                a.download = `${DATA_FOLDER}/${filename}`;
    ...
    return {
        ...
        updateInventoryFromOrder: function(order) {
            ...
            // a proper mapping between menu items and inventory items
